...
import com.twilio.guardrail.languages.{ LA, LanguageAbstraction, ScalaLanguage }
...
object CoreTermInterp extends (CoreTerm ~> CoreTarget) {
  def apply[T, L <: LA](x: CoreTerm[T]): CoreTarget[T] = x match {
    ...
    case ExtractGeneratorSettings(context) =>
      for {
        ...
        generatorSettings <- context.framework.fold(CoreTarget.error[GeneratorSettings[_ <: LA]](NoFramework))({
